...
const App = () => {
  ...
  const [boardData, setBoardData] = useState({ ...newBoardData, solution: getSolution() }); //contine Game from where the user leaves 
  ...
  const enterBoardWord = (word) => {
    ...
    for (var index = 0; index < word.length; index++) { //populate the variables
  ...
  const handleKeyPress = (key) => {  //receive the key from keyboard component
  ...
  return (
    <div className='container'>
      <div className='top'>
        <div className='title'>INFINITE WORDLE</div>
        {boardData.status!=="IN_PROGRESS" && <button className="reset-board" onClick={resetBoard}>{"\u27f3"}</button>}
      </div>
      ...
      <div className='cube'>
        {[0, 1, 2, 3, 4, 5].map((row, rowIndex) => (
          <div className={`cube-row ${boardData && row === boardData.rowIndex && error && "error"}`} key={rowIndex}>{/*wigly effecy*/}
            {
